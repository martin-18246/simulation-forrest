...
public abstract class Animal : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (genesInfo == null)
        {
            genesInfo = BaseTraitSettings.GetRandomGenes(Utilities.NormalDistribution, animalType);
        }
    ...
    protected void ResolveSize()
    {
        switch (currentAge > maturityAge)
        {
            ...
            case false:
                transform.localScale = (baseScale / 2) + (baseScale / 2) * (currentAge / maturityAge);
                break;
        }
    ...
    public class Pregnancy
    {
        ...
        public Pregnancy(UnityEngine.GameObject selfPrefab, Animal parentMother, Animal parentFather, EntityManager entityManager, int litterSize)
        {
            ...
            if (parentMother is Rabbit) _genesChild = CombineGenes(_genesMother, _genesFather, Utilities.NormalDistribution, AnimalType.Rabbit);
            else _genesChild = CombineGenes(_genesMother, _genesFather, Utilities.NormalDistribution, AnimalType.Fox);
